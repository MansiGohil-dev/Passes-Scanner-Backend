...
router.put("/", upload.single("image"), async (req, res) => {
  try {
    ...
    if (req.file) {
      update.imageUrl = `uploads/${req.file.filename}`;
      const pass = await Pass.findOne();
      if (pass && pass.imageUrl) {
        oldImagePath = path.join(__dirname, "../", pass.imageUrl);
      }
    }
...
router.get("/summary", async (req, res) => {
  try {
    const pass = await Pass.findOne();
    const total = pass
      ? pass.count + (await PassShare.aggregate([{ $group: { _id: null, total: { $sum: "$count" } } }]))[0]?.total || 0
      : 0;
    const available = pass ? pass.count : 0;
    const sold = (await PassShare.aggregate([{ $group: { _id: null, total: { $sum: "$count" } } }]))[0]?.total || 0;
    res.json({ total, available, sold });
...
router.get("/sales", async (req, res) => {
  try {
    const shares = await PassShare.find({}, { name: 1, mobile: 1, count: 1, createdAt: 1, token: 1, parentToken: 1, _id: 0 }).sort({
      createdAt: -1,
    });
    res.json(shares);
  } catch (err) {
    console.error("Error in GET /api/passes/sales:", err);
    res.status(500).json({ message: err.message });
  }
});
